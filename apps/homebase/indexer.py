...
def run_historical_sync(db, web3, network_config, alert_func):
    ...
    try:
        # Get network config for fromBlock and lastSyncedBlock
        homebase_network_doc = db.collection("networks").document(doc_name).get()
        if not homebase_network_doc.exists:
            print("WARNING: Homebase network document not found. Skipping historical sync.")
            return

        homebase_network_config = homebase_network_doc.to_dict()
        homebase_network_ref = db.collection("networks").document(doc_name)

        REORG_SAFETY_MARGIN = 100
        from_block_config = homebase_network_config.get('fromBlock', 0)
        last_synced_block = homebase_network_config.get('lastSyncedBlock', 0)
        ...
        if start_block > latest_block:
            ...
            # Import papers dict from setup_papers to get contract instances
            # We need to pass papers as a parameter
            from apps.homebase.paper import Paper as HomebasePaper
            ...
            homebase_network_ref.update({'lastSyncedBlock': latest_block})
